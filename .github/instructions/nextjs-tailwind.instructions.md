---
description: 'Next.js + Tailwind development standards and instructions for b2b-admin-v2'
applyTo: '**/*.tsx, **/*.ts, **/*.jsx, **/*.js, **/*.css'
---

# Next.js + Tailwind Development Instructions

Instructions for the b2b-admin-v2 project built with Next.js 16 (App Router), Tailwind CSS, TypeScript, and shadcn/ui.
...
- `(auth)/` — authentication pages (sign-in and related auth flows)
...
- `features/<domain>/` — Feature-level modules grouped by domain
- `stores/` — Zustand stores (preferences-store, sidebar-store)
- `services/<domain>/` — API logic: `<domain>.type.ts`, `<domain>.schema.ts`, `<domain>.api.ts`, `<domain>.query.ts`
- `apis/` — Shared Axios clients (`base-api.ts`, `sso-api.ts`)
- `configs/` — Application constants (`routes.ts`, `sidebar.ts`, permissions, search)
- `lib/` — Shared utilities (auth, autoform, format, cookies, utils)
...
The `AppHeader` (Search, LayoutControls, LanguageSwitcher, ThemeSwitch, ProfileDropdown) is rendered once in `src/app/admin/layout.tsx` — individual pages do NOT include their own header.
...
- Shared clients are in `src/apis/` (`base-api.ts`, `sso-api.ts`)
- Add new base URLs via `NEXT_PUBLIC_*` env vars and read them directly in client files
...
- Update `src/configs/sidebar.ts` when the new page should appear in admin navigation
...
- Prefer namespaced translation keys (example: `t('dashboard.title')`)
...
- Prefer extending existing components and configs before creating new abstractions
